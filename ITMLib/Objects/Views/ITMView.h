...
namespace ITMLib
{
	/** \brief
	    ...
	    \newhere
	    Also adding semantic images into "view"
	*/
	class ITMView
	{
	public:
		...
		/// Semantic probability of each class for the current frame.
		ITMUShort12Image *semantic;

		/// Semantic probability of each class for the previous frame.
		ITMUShort12Image *semantic_prev;
		...
		ITMView(const ITMRGBDCalib& calibration, Vector2i imgSize_rgb, Vector2i imgSize_d, Vector2i imgSize_semantic, bool useGPU)
		: calib(calibration)
		{
			this->rgb = new ITMUChar4Image(imgSize_rgb, true, useGPU);
			this->rgb_prev = NULL;
			this->semantic = new ITMUShort12Image(imgSize_semantic, true, useGPU);
			this->semantic_prev = NULL;

...
template<class TVoxel, class TIndex>
_CPU_AND_GPU_CODE_ inline void drawPixelSemanticColour(DEVICEPTR(Vector4u) & dest, const CONSTPTR(Vector3f) & point,
	const CONSTPTR(TVoxel) *voxelBlockData, const CONSTPTR(typename TIndex::IndexData) *indexData)
{
	Vector4f sem = VoxelSemanticReader<TVoxel::hasSemanticInformation, TVoxel, TIndex>::interpolate(voxelBlockData, indexData, point);

	dest.x = (uchar)(sem.x * 255.0f);
	dest.y = (uchar)(sem.y * 255.0f);
	dest.z = (uchar)(sem.z * 255.0f);
	dest.w = 255;
}
...
template<class TVoxel, class TIndex>
_CPU_AND_GPU_CODE_ inline void processPixelSemanticColour(DEVICEPTR(Vector4u) &outRendering, const CONSTPTR(Vector3f) & point,
	bool foundPoint, const CONSTPTR(TVoxel) *voxelData, const CONSTPTR(typename TIndex::IndexData) *voxelIndex)
{
	if (foundPoint) drawPixelSemanticColour<TVoxel, TIndex>(outRendering, point, voxelData, voxelIndex);
	else outRendering = Vector4u((uchar)0);
}

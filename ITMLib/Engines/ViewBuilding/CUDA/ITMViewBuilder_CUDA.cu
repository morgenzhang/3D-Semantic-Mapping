...
void ITMViewBuilder_CUDA::UpdateView(ITMView **view_ptr, ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage, ITMUShort12Image *semanticImage, bool useBilateralFilter, bool modelSensorNoise, bool storePreviousImage)
{
	if (*view_ptr == NULL)
	{
		*view_ptr = new ITMView(calib, rgbImage->noDims, rawDepthImage->noDims, semanticImage->noDims, true);
	...
	if (storePreviousImage)
	{
		...
		if (!view->semantic_prev) view->semantic_prev = new ITMUShort12Image(semanticImage->noDims, true, true);
		else view->semantic_prev->SetFrom(view->semantic, MemoryBlock<Vector12us>::CUDA_TO_CUDA);
	}	

	view->rgb->SetFrom(rgbImage, MemoryBlock<Vector4u>::CPU_TO_CUDA);
	view->semantic->SetFrom(semanticImage, MemoryBlock<Vector12us>::CPU_TO_CUDA);
...
void ITMViewBuilder_CUDA::UpdateView(ITMView **view_ptr, ITMUChar4Image *rgbImage, ITMShortImage *depthImage, ITMUShort12Image *semanticImage, bool useBilateralFilter, ITMIMUMeasurement *imuMeasurement, bool modelSensorNoise, bool storePreviousImage)
{
	if (*view_ptr == NULL) 
	{
		*view_ptr = new ITMViewIMU(calib, rgbImage->noDims, depthImage->noDims, semanticImage->noDims, true);
		if (this->shortImage != NULL) delete this->shortImage;
	...
	this->UpdateView(view_ptr, rgbImage, depthImage, semanticImage, useBilateralFilter, modelSensorNoise, storePreviousImage);
}

...
void ITMViewBuilder_CPU::UpdateView(ITMView **view_ptr, ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage, ITMUShort12Image *semanticImage, bool useBilateralFilter, bool modelSensorNoise, bool storePreviousImage)
{ 
	if (*view_ptr == NULL)
	{
		*view_ptr = new ITMView(calib, rgbImage->noDims, rawDepthImage->noDims, semanticImage->noDims, false);
	...
	if (storePreviousImage)
	{
		...
		if (!view->semantic_prev) view->semantic_prev = new ITMUShort12Image(semanticImage->noDims, true, false);
		else view->semantic_prev->SetFrom(view->semantic, MemoryBlock<Vector12us>::CPU_TO_CPU);
	}

	view->rgb->SetFrom(rgbImage, MemoryBlock<Vector4u>::CPU_TO_CPU);
	view->semantic->SetFrom(semanticImage, MemoryBlock<Vector12us>::CPU_TO_CPU);
...
void ITMViewBuilder_CPU::UpdateView(ITMView **view_ptr, ITMUChar4Image *rgbImage, ITMShortImage *depthImage, ITMUShort12Image *semanticImage, bool useBilateralFilter, ITMIMUMeasurement *imuMeasurement, bool modelSensorNoise, bool storePreviousImage)
{
	if (*view_ptr == NULL)
	{
		*view_ptr = new ITMViewIMU(calib, rgbImage->noDims, depthImage->noDims, semanticImage->noDims, false);
		if (this->shortImage != NULL) delete this->shortImage;

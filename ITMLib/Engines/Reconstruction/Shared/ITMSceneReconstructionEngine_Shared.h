...
template<class TVoxel>
_CPU_AND_GPU_CODE_ inline void computeUpdatedVoxelSemanticInfo(DEVICEPTR(TVoxel) &voxel, const THREADPTR(Vector4f) & pt_model, const CONSTPTR(Matrix4f) & M_semantic,
    const CONSTPTR(Vector4f) & projParams_semantic, float mu, uchar maxW, float eta, const CONSTPTR(Vector12us) *semantic, const CONSTPTR(Vector2i) & imgSize)
{
    Vector4f pt_camera;
    Vector2f pt_image;
    Vector12f prob_measure, new_prob, old_prob;
    uchar newW, oldW;
    int locId;

    oldW = voxel.w_semantic;
    old_prob = voxel.prob;
    pt_camera = M_semantic * pt_model;

    pt_image.x = projParams_semantic.x * pt_camera.x / pt_camera.z + projParams_semantic.z;
    pt_image.y = projParams_semantic.y * pt_camera.y / pt_camera.z + projParams_semantic.w;

    if ((pt_image.x < 1) || (pt_image.x > imgSize.x - 2) || (pt_image.y < 1) || (pt_image.y > imgSize.y - 2)) return;

    locId = (int)(pt_image.x + 0.5f) + (int)(pt_image.y + 0.5f) * imgSize.x;
    prob_measure = semantic[locId].toFloat() / 20000.0f;
	newW = 1;

	//// different methods of updating semantic prob
    /** doing averaging according to the observed number of visible voxel */
//	new_prob = oldW * old_prob + newW * prob_measure;
//    newW = oldW + newW;
//    new_prob /= newW;
//    newW = MIN(newW, maxW);

    /** doing recursive bayesian on semantic prob of visible voxel */
    new_prob = (oldW == 0) ? prob_measure : old_prob * prob_measure;
    newW = oldW + newW;
    new_prob /= sum(new_prob);
    newW = MIN(newW, maxW);

    voxel.prob = new_prob;
    voxel.w_semantic = (uchar)newW;
}

template<bool hasColor, bool hasConfidence, bool hasSemantic, class TVoxel> struct ComputeUpdatedVoxelInfo;
...
struct ComputeUpdatedVoxelInfo<true, false, true, TVoxel> {
    _CPU_AND_GPU_CODE_ static void compute(DEVICEPTR(TVoxel) & voxel, const THREADPTR(Vector4f) & pt_model,
        ...
        const THREADPTR(Matrix4f) & M_semantic, const THREADPTR(Vector4f) & projParams_semantic,
        float mu, int maxW,
        ...
        const CONSTPTR(Vector12us) *semantic, const THREADPTR(Vector2i) & imgSize_semantic)
    {
        ...
        computeUpdatedVoxelSemanticInfo(voxel, pt_model, M_semantic, projParams_semantic, mu, maxW, eta, semantic, imgSize_semantic);
    }
};
...
_CPU_AND_GPU_CODE_ inline void buildHashAllocAndVisibleTypePP(DEVICEPTR(uchar) *entriesAllocType, DEVICEPTR(uchar) *entriesVisibleType, int x, int y,
	DEVICEPTR(Vector4s) *blockCoords, const CONSTPTR(float) *depth, const CONSTPTR(Vector12us) *semantic, Matrix4f invM_d, Vector4f projParams_d, float mu, Vector2i imgSize,
	float oneOverVoxelSize, const CONSTPTR(ITMHashEntry) *hashTable, float viewFrustum_min, float viewFrustum_max)
{
	float depth_measure; Vector12f semantic_measure; unsigned int hashIdx; int noSteps; int class_id = -1; float max_tmp = 0.0;
	Vector4f pt_camera_f; Vector3f point_e, point, direction; Vector3s blockPos;

	depth_measure = depth[x + y * imgSize.x];
	semantic_measure = semantic[x + y * imgSize.x].toFloat() / 20000.0f;
	for (int i = 0; i < 12; ++i) {
		if (semantic_measure.v[i] > max_tmp) {
			max_tmp = semantic_measure.v[i];
			class_id = i;
		}
	}
	// not allocate for depth out of defined visible range
	// not allocate for semantic labeled as sky
	if (class_id == 7 || depth_measure <= 0 || (depth_measure - mu) < 0 || (depth_measure - mu) < viewFrustum_min || (depth_measure + mu) > viewFrustum_max) return;

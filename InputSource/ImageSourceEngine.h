...
namespace InputSource {
	...
	public:
		 ...
		 *
		 * \newhere
		 * \param semantic  An image into which to store the next semantic segmented image.
		 */
		virtual void getImages(ITMUChar4Image *rgb, ITMShortImage *rawDepth, ITMUShort12Image *semantic) = 0;
		...
		/**
		 * \newhere
		 * \brief Gets the size of the next semantic segmented image (if any).
		 *
		 * \pre     hasMoreImages()
		 * \return  The size of the next semantic segmented image (if any), or Vector2i(0,0) otherwise.
		 */
		virtual Vector2i getSemanticImageSize(void) const = 0;
	...
	private:
		...
		char semanticImageMask[BUF_SIZE];

	public:
		ImageMaskPathGenerator(const char *rgbImageMask, const char *depthImageMask, const char *semanticImageMask);
		std::string getRgbImagePath(size_t currentFrameNo) const;
		std::string getDepthImagePath(size_t currentFrameNo) const;
		std::string getSemanticImagePath(size_t currentFrameNo) const;
	};
	...
	private:
		...
		std::vector<std::string> semanticImagePaths;

		size_t imageCount() const;

	public:
		ImageListPathGenerator(const std::vector<std::string>& rgbImagePaths_, const std::vector<std::string>& depthImagePaths_, const std::vector<std::string>& SemanticImagePaths_);
		std::string getRgbImagePath(size_t currentFrameNo) const;
		std::string getDepthImagePath(size_t currentFrameNo) const;
		std::string getSemanticImagePath(size_t currentFrameNo) const;
	...
	private:
		...
		ITMUShort12Image *cached_semantic;
	...
	public:
		...
		void getImages(ITMUChar4Image *rgb, ITMShortImage *rawDepth, ITMUShort12Image *semantic);
		Vector2i getDepthImageSize(void) const;
		Vector2i getRGBImageSize(void) const;
		Vector2i getSemanticImageSize(void) const;
	};
	...
	public:
		...
		Vector2i getSemanticImageSize(void) const { return imgSize; }
	};
	...
	private:
		...
		mutable ITMUShort12Image *cached_semantic;
	...
	public:
		RawFileReader(const char *calibFilename, const char *rgbImageMask, const char *depthImageMask, const char *semanticImageMask, Vector2i setImageSize, float ratio);
		~RawFileReader() { }

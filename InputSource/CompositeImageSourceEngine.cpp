...
void CompositeImageSourceEngine::getImages(ITMUChar4Image *rgb, ITMShortImage *rawDepth,ITMUShort12Image *semantic)
{
  if(advanceToNextImages()) m_subengines[m_curSubengineIndex]->getImages(rgb, rawDepth, semantic);
}
...
Vector2i CompositeImageSourceEngine::getSemanticImageSize(void) const
{
  // There is an assumption being made that the RGB image sizes for all the sub-engines are the same,
  // although this is not currently being enforced.
  if(!m_subengines.empty()) return m_subengines[0]->getSemanticImageSize();
  else throw std::runtime_error("Cannot get the Semantic segmented image size from an empty composite image source engine");
}

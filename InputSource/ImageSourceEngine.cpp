...
ImageMaskPathGenerator::ImageMaskPathGenerator(const char *rgbImageMask_, const char *depthImageMask_,const char *semanticImageMask_)
{
	strncpy(rgbImageMask, rgbImageMask_, BUF_SIZE);
	strncpy(depthImageMask, depthImageMask_, BUF_SIZE);
	strncpy(semanticImageMask, semanticImageMask_, BUF_SIZE);
}
...
std::string ImageMaskPathGenerator::getSemanticImagePath(size_t currentFrameNo) const
{
	char str[BUF_SIZE];
	sprintf(str, semanticImageMask, currentFrameNo);
	return std::string(str);
}

ImageListPathGenerator::ImageListPathGenerator(const std::vector<std::string>& rgbImagePaths_, const std::vector<std::string>& depthImagePaths_, const std::vector<std::string>& semanticImagePaths_)
	: depthImagePaths(depthImagePaths_),
	  rgbImagePaths(rgbImagePaths_),
	  semanticImagePaths(semanticImagePaths_)
{
...
std::string ImageListPathGenerator::getSemanticImagePath(size_t currentFrameNo) const
{
	return currentFrameNo < imageCount() ? semanticImagePaths[currentFrameNo] : "";
}
...
// mainly used imagesource class: ImageFileReader
	  ...
	  pathGenerator(pathGenerator_)
{
    ...
    // define a new kind of file format for storing semantic data with 20 channels
    //cached_semantic = new ITMFloat20Image(true, false);
    cached_semantic = new ITMUShort12Image(true, false);
...
void ImageFileReader<PathGenerator>::loadIntoCache(void) const
{
	...
	// Todo here, must modify corresponding input IO function for semantic data
	std::string semanticPath = pathGenerator.getSemanticImagePath(currentFrameNo);
	if (!ReadImageFromFile(cached_semantic, semanticPath.c_str()))
	{
		if (cached_semantic->noDims.x > 0) cacheIsValid = false;
		printf("error reading file '%s'\n", semanticPath.c_str());
	}

	if ((cached_rgb->noDims.x <= 0) && (cached_depth->noDims.x <= 0) && (cached_semantic->noDims.x <= 0)) cacheIsValid = false;
...
template <typename PathGenerator>
void ImageFileReader<PathGenerator>::getImages(ITMUChar4Image *rgb, ITMShortImage *rawDepth, ITMUShort12Image *semantic)
{
	loadIntoCache();
	...
	semantic->SetFrom(cached_semantic, ORUtils::MemoryBlock<Vector12us>::CPU_TO_CPU);
//	for (int i = 0; i < 20; ++i) {
//		printf("value of class %i of pixel idx 0 in cach is %i\n", i, cached_semantic->GetData(MEMORYDEVICE_CPU)[0].v[i]);
//	}
	++currentFrameNo;
}
...
template <typename PathGenerator>
Vector2i ImageFileReader<PathGenerator>::getSemanticImageSize(void) const
{
	loadIntoCache();
	return cached_semantic->noDims;
}
...
RawFileReader::RawFileReader(const char *calibFilename, const char *rgbImageMask, const char *depthImageMask, const char *semanticImageMask, Vector2i setImageSize, float ratio)
	: BaseImageSourceEngine(calibFilename)
{
	this->imgSize = setImageSize;
	this->ResizeIntrinsics(calib.intrinsics_d, ratio);
	...
	strncpy(this->semanticImageMask, semanticImageMask, BUF_SIZE);
...
void RawFileReader::loadIntoCache(void) const
{
	...
	//TODO> make nicer
    if (cached_rgb == NULL && cached_depth == NULL && cached_semantic == NULL)
    {
        cached_rgb = new ITMUChar4Image(imgSize, MEMORYDEVICE_CPU);
        cached_depth = new ITMShortImage(imgSize, MEMORYDEVICE_CPU);
		// cached_semantic = new ITMFloat20Image(imgSize, MEMORYDEVICE_CPU);
        cached_semantic = new ITMUShort12Image(imgSize, MEMORYDEVICE_CPU);
    }
	...
	sprintf(str, semanticImageMask, currentFrameNo); success = false;
	f = fopen(str, "rb");
	if (f)
	{
		size_t tmp = fread(cached_semantic->GetData(MEMORYDEVICE_CPU), sizeof(Vector12us), imgSize.x * imgSize.y, f);
		fclose(f);
		if (tmp == (size_t)imgSize.x * imgSize.y) success = true;
	}
	if (!success)
	{
		delete cached_semantic; cached_semantic = NULL;
		printf("error reading file '%s'\n", str);
	}
}
...
bool RawFileReader::hasMoreImages(void) const
{
	...
	return ((cached_rgb != NULL) || (cached_depth != NULL) || (cached_semantic != NULL));
}

void RawFileReader::getImages(ITMUChar4Image *rgb, ITMShortImage *rawDepth, ITMUShort12Image *semantic)
{
...
void BlankImageGenerator::getImages(ITMUChar4Image *rgb, ITMShortImage *rawDepth, ITMUShort12Image *semantic)
{
	rgb->Clear();
	rawDepth->Clear();
	semantic->Clear();

...
// Todo here, add another file-type reader
bool ReadImageFromFile(ORUtils::Image<ORUtils::VectorX<unsigned short, 12> > *image, const char *fileName)
{
	PNGReaderData pngData;
	bool usepng = false;

	int xsize, ysize, single_ysize;
	bool binary;
	FILE *f = fopen(fileName, "rb");
	if (f == NULL) return false;
	FormatType type = pnm_readheader(f, &xsize, &ysize, &binary);
	...
	single_ysize = ysize / 12;
    ORUtils::Vector2<int> newSize(xsize, single_ysize);
    image->ChangeDims(newSize);
    ORUtils::VectorX<unsigned short, 12> *dataPtr = image->GetData(MEMORYDEVICE_CPU);

    for (int i = 0; i < image->noDims.x*image->noDims.y; ++i)
    {
        for (int j = 0; j < 12; ++j)
        {
            dataPtr[i].v[j] = (data[i + j*xsize*single_ysize] << 8) | ((data[i + j*xsize*single_ysize] >> 8) & 255);
        }
    }

...
namespace ORUtils {
	...
	// typically useful for customer defined data
	...
	public:
		...
		_CPU_AND_GPU_CODE_ inline VectorX<unsigned short, s> toUShortRound() const {
			VectorX<unsigned short, s> retv;
			for (int i = 0; i < s; i++) retv[i] = (unsigned short)ROUND(this->v[i]);
			return retv;
		}
	...
	// compute the summary of all components

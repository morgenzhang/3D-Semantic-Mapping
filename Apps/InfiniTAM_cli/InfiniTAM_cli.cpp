...
try
{
	...
	const char *imagesource_part4 = NULL;
	...
	do {
		...
		++arg;
		if (argv[arg] != NULL) imagesource_part4 = argv[arg]; else break;
	} while (false);
...
//	}
	if (imagesource_part2 != NULL)
	{
		if (imagesource_part4 == NULL)
		{
			printf("using rgb images: %s\nusing depth images: %s\nusing semantic images: %s\n", imagesource_part1, imagesource_part2, imagesource_part3);
			ImageMaskPathGenerator pathGenerator(imagesource_part1, imagesource_part2, imagesource_part3);
			imageSource = new ImageFileReader<ImageMaskPathGenerator>(calibFile, pathGenerator);
		}
		else
		{
			printf("using rgb images: %s\nusing depth images: %s\nusing semantic images: %s\nusing imu data: %s\n", imagesource_part1, imagesource_part2, imagesource_part3, imagesource_part4);
			imageSource = new RawFileReader(calibFile, imagesource_part1, imagesource_part2, imagesource_part3, Vector2i(320, 240), 0.5f);

...
/** Create a default source of depth images from a list of command line
    ...
    use, @para arg2 the colour images, @para arg3 the depth images @para
    arg4 the Semantic data and @para arg5 the IMU images. If images are
    omitted, some live sources will be tried.
*/
static void CreateDefaultImageSource(ImageSourceEngine* & imageSource, IMUSourceEngine* & imuSource, const char *arg1, const char *arg2, const char *arg3, const char *arg4, const char *arg5)
{
	...
	const char *filename_semantic = arg4;
	...
	{
		//// Upadate here
		printf("using rgb images: %s\nusing depth images: %s\nusing semantic images: %s\n", filename1, filename2, filename_semantic);
		if (filename_imu == NULL)
		{
			ImageMaskPathGenerator pathGenerator(filename1, filename2, filename_semantic);
			imageSource = new ImageFileReader<ImageMaskPathGenerator>(calibFile, pathGenerator);
		}
		else
		{
			printf("using imu data: %s\n", filename_imu);
			imageSource = new RawFileReader(calibFile, filename1, filename2, filename_semantic, Vector2i(320, 240), 0.5f);
	...
	/** Not used in my project, only import source images:
	 *      * RGB images
	 *      * Depth images
	 *      * Semantic data (probability of different class) */

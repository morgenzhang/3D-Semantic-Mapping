...
void UIEngine::glutDisplayFunction()
{
	...
	{
	case 0: glColor3f(1.0f, 0.0f, 0.0f); break;  // failure,      color: red
	case 1: glColor3f(1.0f, 1.0f, 0.0f); break;  // poor,         color: yellow
	case 2: glColor3f(0.0f, 1.0f, 0.0f); break;  // good,         color: green
	default: glColor3f(1.0f, 1.0f, 1.0f); break; // relocalising, color: white
	}
...
void UIEngine::glutKeyUpFunction(unsigned char key, int x, int y)
{
	...
	case 'v':
		if ((uiEngine->rgbVideoWriter != NULL) || (uiEngine->depthVideoWriter != NULL))
		{
			...
			// delete uiEngine->semanticVideoWriter;
			uiEngine->rgbVideoWriter = NULL;
			uiEngine->depthVideoWriter = NULL;
			// uiEngine->semanticVideoWriter = NULL;
		}
		else
		{
			...
			// uiEngine->semanticVideoWriter = new FFMPEGWriter();
		}
		break;
	case 'e':
	case 27: // esc key
	...
	const char *outFolder, ITMLibSettings::DeviceType deviceType)
{
	...
	this->colourModes_main.push_back(UIColourMode("semantic paint", ITMMainEngine::InfiniTAM_IMAGE_COLOUR_FROM_SEMANTIC));
	...
	this->colourModes_freeview.push_back(UIColourMode("semantic paint", ITMMainEngine::InfiniTAM_IMAGE_FREECAMERA_COLOUR_FROM_SEMANTIC));
	...
	winReg[1] = Vector4f(0.666f, h2, 1.0f, 1.0f);   // Side sub window 1, display depth image
	winReg[2] = Vector4f(0.666f, h1, 1.0f, h2);     // Side sub window 2, display RGB image
	//winReg[3] = Vector4f(0.0f, h2, 0.665f, 1.0f);   // Main render 2
	//winReg[3] = Vector4f(0.665f, h1, 1.0f, h2);     // Side Sub window 3, display semantic result

	this->isRecording = false;
	this->currentFrameNo = 0;
	this->rgbVideoWriter = NULL;
	this->depthVideoWriter = NULL;
    // this->semanticVideoWriter = NULL;
    ...
    inputSemanticImage = new ITMUShort12Image(imageSource->getSemanticImageSize(), true, allocateGPU);
	...
	//outImageType[3] = ITMMainEngine::InfiniTAM_IMAGE_COLOUR_FROM_VOLUME;
    // outImageType[3] = ITMMainEngine::InfiniTAM_IMAGE_ORIGINAL_SEMANTIC; // semantic
...
void UIEngine::ProcessFrame()
{
	...
	imageSource->getImages(inputRGBImage, inputRawDepthImage, inputSemanticImage);
...
//    if ((semanticVideoWriter != NULL) && (inputSemanticImage->noDims.x != 0)) {
//        if (!semanticVideoWriter->isOpen()) semanticVideoWriter->open("out_semantic.avi", inputSemanticImage->noDims.x, inputSemanticImage->noDims.y, false, 30);
//        semanticVideoWriter->writeFrame(inputSemanticImage);
//    }
	...
	if (imuSource != NULL) trackerResult = mainEngine->ProcessFrame(inputRGBImage, inputRawDepthImage, inputSemanticImage, inputIMUMeasurement);
	else trackerResult = mainEngine->ProcessFrame(inputRGBImage, inputRawDepthImage, inputSemanticImage);
...
void UIEngine::Shutdown()
{
    ...
    // if (semanticVideoWriter != NULL) delete semanticVideoWriter;
